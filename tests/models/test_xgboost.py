...
def test_fit_predict_proba_returns_valid_probabilities() -> None:
    df = _synthetic_frame()
    model = XGBoostCalibratedModel(feature_names=FEATURES)
...
def test_save_and_load_round_trips_predictions(tmp_path) -> None:
    ...
    model.save(tmp_path)
    reloaded = XGBoostCalibratedModel.load(tmp_path)
...
def test_class_weight_balanced_sets_scale_pos_weight() -> None:
    df = _synthetic_frame(n=200)
    model = XGBoostCalibratedModel(feature_names=FEATURES, class_weight="balanced")
    model.fit(df, df["target"])
    assert model._base.get_params()["scale_pos_weight"] != 1.0
